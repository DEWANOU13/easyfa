...
@section('content')
    ...
    <section>
        <form id="formcreate" action="{{ route('store_reception_acheminement') }}" method="post">
            @csrf
            @if ($errors->any())
                ...
                <div class="card-body">
                    <fieldset class="border p-3 rounded-3" id="table">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <label class="form-label fw-bold" for="net_a_payer">Qté Acheminer</label>
                                <div class="input-group input-group-sm mb-3">
                                    <input step="0.01" type="text" name=""
                                        class="form-control net-a-payer-input" value="{{ old('net_a_payer') }}"
                                        id="net_a_payer" placeholder="Qté Acheminer..." readonly>
                                </div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <label class="form-label fw-bold" for="montant">Quantité</label>
                                <div class="input-group input-group-sm mb-3">
    ...
    $(document).ready(function() {
        ...
        $('#num_facture').on('change', function() {
            ...
            if (selectedCategoryId) {
                $.ajax({
                    url: '/get-acheminement-reception',
                    type: 'POST',
                    headers: {
                        ...
                        if (response.length > 0) {
                            response.forEach(function(produit) {
                                $('#produit').append('<option value="' +
                                    produit.id + '|' +
                                    produit.Id_Stock + '|' + produit.Designation +
                                    '">' +
