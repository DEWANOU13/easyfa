...
<body>
    ...
    @if ($reponse === 'ACTIF' || $reponse === 'INACTIF' || $reponse === 'PRESTATION')
        @if ($reponse === 'ACTIF')
            <div class="titre" style="padding-top: 20px;">LISTES DES PRODUITS ACTIFS</div>
        @endif

        @if ($reponse === 'INACTIF')
            <div class="titre" style="padding-top: 20px;">LISTES DES PRODUITS INACTIFS</div>
        @endif
        @if ($reponse === 'PRESTATION')
            <div class="titre" style="padding-top: 20px;">LISTES DES PRESTATIONS</div>
        @endif
    @endif
    <br>
    ...
    @foreach ($getCategorieProduit as $categorie_produit)
        <div class="entete">
            <h3>{{ $categorie_produit->Libelle }}</h3>
        </div>
        <div class="table">
            <table class="tableLigne">
                @php
                    $nombre = 0;
                @endphp
                @if (count($getProduit) > 0)
                    <thead>
                        <th style="background-color: #3232df;font-weight: bold; color: white; ">Référence</th>
                        <th style="background-color: #3232df;font-weight: bold; color: white; ">Désignation</th>
                        <th style="background-color: #3232df;font-weight: bold; color: white; ">Unite Comptage</th>
                    </thead>
                @endif
                <tbody>
                    @foreach ($getProduit as $value)
                        @if ($categorie_produit->Id_Categorie === $value->Id_Categorie)
                            <tr>
                                <td>{{ $value->Reference }}</td>
                                <td>{{ $value->Designation }}</td>
                                <td>{{ $value->Libelle_Comptage }}</td>p
                        @endif
                        @php
                            $nombre += 1;
                        @endphp
    ...
    @endforeach
    <div class="nombre">Nombre de produit: <strong>{{ $nombre }}</strong></div>
    {{-- @endforeach --}}
    <style>
        .box1 {
            width: 50%;
