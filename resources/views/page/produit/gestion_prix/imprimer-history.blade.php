...
<body>
    ...
    <div class="titre" style="padding-top: 20px;">LISTES HISTORIQUES PRIX</div>
    <br>
    {{-- @foreach ($entree_produits as $entree_produit) --}}
        ...
        <div class="table" style="margin-top: 15px;">
            <table class="tableLigne">
                <thead>
                    <th style="background-color: #3232df;font-weight: bold; color: white; ">Date</th>
                    ...
                    <th style="background-color: #3232df;font-weight: bold; color: white; ">Prix</th>
                </thead>
                <tbody>
                    @foreach ($gestions_prix_history as $item)
                        <tr>
                            <td>{{ $item->date_changement_prix }}</td>
                            <td>{{ $item->Designation }}</td>
                            <td>{{ $item->NomAgence }}</td>
                            <td>{{ $item->Libelle }}</td>
                            <td>{{  number_format(intval($item->prix), 0, ',', ' ')  }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
