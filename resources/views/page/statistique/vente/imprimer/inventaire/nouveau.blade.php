            ...
            <div class="row my-4">
                <div class="col-md-12">
                    <form id="formcreate" class="row gx-3 gy-2 d-flex align-items-center justify-content-center "
                        action="{{ route('store.inventaire') }}" methode='post' id="myForm">
                        @csrf
                        @if ($errors->any())
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#categorie').on('change', function() {
                ...
                if (selectedCategoryId) {
                    $.ajax({
                        url: '/get-products-by-category-inventaire/' +
        ...
        $(document).ready(function() {
            ...
            $('#magasin').on('change', function() {
                ...
                if (selectedCategoryId) {
                    $.ajax({
                        url: '/get-category-by-magasin-inventaire/' +
                            selectedCategoryId, // URL à adapter selon tes routes
                        type: 'GET',
                        success: function(response) {
                            // Ajouter les produits retournés dans le menu déroulant
                            if (response.length > 0) {
                                response.forEach(function(category) {
                                    $('#categorie').append('<option value="' +
                                        category.Id_Magasin + '-' +
                                        category.Id_Categorie + '-' + category
                                        .Libelle + '" ' +
                                        '>' + category.Libelle +
                                        '</option>'
                                    );
