@extends('layouts.master', ['title' => 'Caisse'])
...
@section('content')
                    ...
                    @can('ouvrir-caisse')
                        <li>
                            <a type="button" class="dropdown-item" data-bs-toggle="modal"
                                data-bs-target="#creeCatEmballage">Nouvelle caisse</a>
                        </li>
                    @endcan

                    @can('fermer-caisse')
                        <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#fermerCaisseM"
                                type="button">Fermer caisse</a></li>
                    @endcan
                    @can('exporter-caisse-pdf')
                        <li>
                            <a type="button" class="dropdown-item" id="caisseExportPdfLink" target="_blank">Exporter en PDF</a>

                        </li>
                    @endcan
                    @can('exporter-caisse-excel')
                        <li>
                            <a type="button" class="dropdown-item" id="caisseExportExcelLink">Exporter en Excel</a>
                        </li>
                    @endcan
                </ul>
            </div>
    ...
    <div class="modal fade" id="fermerCaisseM" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        aria-labelledby="fermerCaisseMLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">

                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="fermerCaisseMLabel">Nouvelle caisse</h1>
                ...
                <div class="modal-footer">
                    ...
                    <a type="button" id="modifierProformaLink" class="btn btn-primary">Oui fermer</a>
                </div>
                    ...
                    <div class="row mt-4 mb-5">
                        <div class="col-md-2 border">
                            <form class="d-flex align-items-center mt-2" id="filterForm"
                                action="{{ route('filterCaisse') }}" method="GET">
                                <div class="row">
                        ...
                        <div class="col-md-10">
                            <div class="card m-b-30 my-3">
                                <div class="card-header" style="{{ background_color_2() }}">
                                    <h3 class="mt-2  d-inline-block text-dark">Liste des caisses</h3>
                                </div>
                                            ...
                                            <tbody>
                                                @forelse ($listeCaisse as $caisse)
                                                    <tr style="cursor:pointer;" class="clickable-row "
                                                        data-url="{{ route('getDetailCaisse', ['id' => $caisse->id]) }}">
                            ...
                            <div class="card m-b-30 my-3">
                                <div class="card-header" style="{{ background_color_2() }}">
                                    <h3 class="mt-2  d-inline-block text-dark">Historique mouvements de la caisse</h3>
                                    {{--   <div class="float-right">
                                        <a href="{{ route('caisseExportPdf') }}" class="btn btn-danger">Exporter en PDF</a>
                                        <a href="{{ route('caisseExportExcel') }}" class="btn btn-success">Exporter en Excel</a>
                                    </div> --}}
                                </div>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            @if ($detail->statut == 'ANNULEE')
                                                                <span class="status-fermee">ANNULEE</span>
                                                            @endif
    ...
    <script>
        ...
        function applyTableEvents() {
            ...
            $('#proformaTable').on('click', '.clickable-row', function() {
                ...
                var exporterUrl = "{{ route('caisseExportPdf', ['id' => ':caisseId']) }}";

                var excelUrl = "{{ route('caisseExportExcel', ['id' => ':caisseId']) }}";
                var pdfProformaUrl = "{{ route('PDFProformaA4', ['id' => ':caisseId']) }}";
                fermerCaisse = fermerCaisse.replace(':caisseId', caisseId);
                exporterUrl = exporterUrl.replace(':caisseId', caisseId);
                excelUrl = excelUrl.replace(':caisseId', caisseId);
                pdfProformaUrl = pdfProformaUrl.replace(':caisseId', caisseId);
                $('#modifierProformaLink').attr('href', fermerCaisse);
                $('#caisseExportPdfLink').attr('href', exporterUrl);
                $('#caisseExportExcelLink').attr('href', excelUrl);
