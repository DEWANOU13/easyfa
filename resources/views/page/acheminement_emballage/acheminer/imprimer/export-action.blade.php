...
<table>
    <tr>
        <td colspan="7" style="text-align: center; font-weight: bold; font-size:18px;">BORDEREAU ACHEMINEMENT EMBALLAGE
        </td>
    </tr>
</table>
@foreach ($acheminement as $acheminement)
    <table>
        <tr>
    ...
    <table>
        <tr>
            ...
            <th style="background-color: #3232df;font-weight: bold; text-align: center; color: white; ">Qté_Acheminée</th>
        ...
        <tbody>
            @foreach ($acheminer as $value)
                <tr>
                    <td>{{ $value->Reference }}</td>
                    <td>{{ $value->Designation }}</td>
                    <td>{{ $value->Libelle }}</td>
                    <td>{{ $value->NomMagasin }}</td>
                    <td>{{ $value->Qte_acheminee }}</td>
                    <td>{{ 0 }}</td>
                    <td>{{ $value->Qte_acheminee}}</td>
                </tr>
            @endforeach
