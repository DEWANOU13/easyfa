@extends('layouts.master', ['title' => 'Acheminer'])
@section('content')
    @include('layouts.partials.entete-page', [
        'infos1' => 'Acheminer emballage',
        'infos2' => 'Acheminer emballage',
        'infos3' => 'Liste',
    ])
        ...
        <div class="col text-end">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" style="z-index: 2000;">
                    @can('effectuer-acheminement-emballage')
                        <li><a class="dropdown-item" type="button"
                                href="{{ route('nouveau_acheminer_emballage') }}">Nouveau</a>
                        </li>
                    @endcan
                    <form action="{{ route('acheminement_imprimer_action_emballage') }}" method="POST" target="_blank">
                        @csrf
                        <input type="hidden" id="value" name="id_acheminement">
                        ...
                        <div class="col-md-2">
                            <div class="row border">
                                <form class="d-flex align-items-center mt-2" id="filterForm"
                                    action="{{ route('filterAchminerEmballage') }}" method="GET">
                                    <div class="row">
                        ...
                        <div class="col-md-10">
                            <div class="card m-b-30 my-3">
                                <div class="card-header" style="{{ background_color_2() }}">
                                    <h3 class="mt-2  d-inline-block text-dark">Liste des acheminements emballage</h3>
                                </div>
                                <div class="card-body responsive-2">
                                    <div class="table-responsive">
                                        <table id="proformaTable"
                                            ...
                                            <tbody>
                                                @forelse($acheminements as $acheminement)
                                                    <tr style="cursor:pointer" class="clickable-row"
                                                        data-url="{{ route('get.acheminer_by_acheminement_emballage', ['id' => $acheminement->id]) }}">
                                                        <td hidden class="entree-produit"><input type="hidden"
                                                                value="{{ $acheminement->id }}"></td>
                                                        <td class="entree-produit">{{ $acheminement->NomAgence }}
                                                        </td>
                                                        <td class="entree-produit">
                                                            {{ \Carbon\Carbon::parse($acheminement->Date_acheminement)->format('d/m/Y H:i') }}
                                                        </td>
                                                        <td class="entree-produit">
                                                            {{ $acheminement->Reference_acheminement }}</td>
                                                        <td class="entree-produit">
                                                            {{ $acheminement->Statut_acheminement }}</td>
                                                        <td class="entree-produit">
                                                            {{ $acheminement->Observations }}</td>
                                                        <td class="entree-produit">{{ $acheminement->name }}</td>
                                                    </tr>
                                                @empty
                                                @endforelse
                                            </tbody>
                            ...
                            <div class="card m-b-30 my-3">
                                <div class="card-header" style="{{ background_color_2() }}">
                                    <h3 class="mt-2  d-inline-block text-dark">Détails acheminement</h3>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive responsive-1">
                                        <div class="col-md-12">
                                            <table id=""
                                                ...
                                                <tbody>
                                                    @if (isset($acheminers))
                                                        @forelse($acheminers as $acheminer)
                                                            <tr style="cursor:pointer" class="clickable-row">
                                                                <td class="entree-produit">
                                                                    {{ $acheminer->Reference }}</td>
                                                                <td class="entree-produit">
                                                                    {{ $acheminer->Designation }}</td>
                                                                <td class="entree-produit">{{ $acheminer->Libelle }}
                                                                </td>
                                                                <td class="entree-produit">
                                                                    {{ $acheminer->NomMagasin }}
                                                                </td>
                                                                <td class="entree-produit">
                                                                    {{ $acheminer->Qte_acheminee }}
                                                                </td>
                                                                <td class="entree-produit">
                                                                    {{ $acheminer->Qte_Receptionnee }}</td>
                    ...
                    <div class="row my-4">
                        <div class="col-md-8 offset-md-2 mb-5">
                            ...
                            <form class="row gx-3 gy-2 pb-4" method="POST"
                                action="{{ route('imprimer_acheminement_emballage') }}" target="_blank">
                                @csrf
                                <div class="col-sm-6">
