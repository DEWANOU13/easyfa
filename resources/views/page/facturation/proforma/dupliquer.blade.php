@extends('layouts.master', ['title' => 'Dupliquer un Proforma'])
@section('content')
...
<section>
  ...
  <form id="formX" action="{{ isset($infoProforma) ? route('dupliquerStoreProforma', $infoProforma->id) : route('storeProforma') }}" method="POST" class="">
    @csrf
    @method('PUT')
      ...
      <div class="card-body">
        <fieldset class="border p-3 rounded-3">
          ...
          <div class="row">
            ...
            <input type="hidden" name="hiddenAib_deductible" id="hiddenAib_deductible" value="{{ isset($infoProforma) ? $infoProforma->Aib_deductible : '' }}">
    ...
    <div class="card m-b-30">
      <div class="card-body">
        <fieldset class="border p-3 rounded-3">
          ...
          <button type="button" id="add" name="add" class="btn w-auto float-end text-white" style="{{ background_color_1() }}"><span class="fw-bold">+</span> Ajouter</button>
        </fieldset>
      </div>
    </div>
    <div class="card m-b-30">
      ...
      <div class="modal-dialog g modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            Souhaitez-vous vraiment dupliquer ce proforma ?
          </div>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $.ajax({
      ...
      , success: function(response) {
        ...
        if (response.detailProforma && response.detailProforma.length > 0) {
          ...
          /* $('#reference').val(firstProduct.Reference);
          $('#designation').val(firstProduct.designation);
          $('#taxe_id').val(firstProduct.taxe_id);
          $('#quantity').val(firstProduct.Qte);
          $('#pu_HT').val(firstProduct.Prix_unitaire_HT);
          $('#Taux_remise').val(firstProduct.Taux_remise);
          $('#pu_HT_net').val(firstProduct.pu_HT_net);
          $('#pu_TTC').val(firstProduct.pu_TTC); */
          ...
          for (var i = 0; i < response.detailProforma.length; i++) {
            ...
            var montantHT = quantite * pu_HT * (1 - tauxRemise / 100);
    ...
    //   console.log('me voici taxeLettre', taxeLettre);
      ...
      totalHT_B -= parseFloat(parseFloat(supprimerMontantTaxeB).toFixed(0));
      totalTVA_B -= parseFloat(parseFloat(supprimerTVA_B).toFixed(0));
      $('#totalHT_B').val(totalHT_B.toFixed(0));
      $('#totalTVA_B').val(totalTVA_B.toFixed(0));
      ...
      //voir = parseFloat(supprimerAibfacturer).toFixed(0) + parseFloat(supprimerAibfacturer2).toFixed(0);

      var voir = parseFloat(supprimerAibfacturer.toFixed(0)) + parseFloat(supprimerAibfacturer2.toFixed(0));
      console.log('supprimerAibfacturer2', supprimerAibfacturer2);

      var Net_a_payer_deja_facturer_x = $('#Net_a_payer').val();
      var Net_a_payer_deja_facturer = parseFloat(parseFloat(Net_a_payer_deja_facturer_x).toFixed(0));
