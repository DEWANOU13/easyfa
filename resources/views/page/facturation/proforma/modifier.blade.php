@extends('layouts.master', ['title' => 'Modifier Proforma'])
@section('content')
...
<section>
  ...
  <form id="formX" action="{{ isset($infoProforma) ? route('updateProforma', $infoProforma->id) : route('storeProforma') }}" method="POST" class="">
    @csrf
    @method('PUT')
    <div class="card-body">
      @if ($errors->any())
        ...
        <fieldset class="border p-3 rounded-3">
          ...
          <div class="row">
            <div class="form-group col-md-6">
              ...
              <select class="form-select select-readonly" name="client_id" required id="client">
                <option></option>
            ...
            <script>
                $(document).ready(function() {
                    // Empêcher l'interaction avec le champ <select>
                    $('#client').on('focus mousedown', function(e) {
                        e.preventDefault(); // Empêche le focus ou le clic sur le champ
                    });
                });
            ...
            <div class="form-group col-md-6">
              <label for="validationTextarea" class="form-label fw-bold position-relative">Validité
                <label for="validationTextarea" class="form-label fw-bold position-relative">Client
                    <span class="position-absolute top-0 start-100 translate-middle p-2 text-danger ms-1 mt-2">*
                    </span>
                  </label>
              </label>
              <input type="text" name="Validite" class="form-control" value="{{ isset($infoProforma) ? $infoProforma->Validite : '' }}" required>
            </div>
            <div class="form-group col-md-6">
            ...
            <div class="form-group col-md-3">
              ...
              <div class="input-group input-group-sm mb-3">
                ...
                <select name="" type="text" class="form-select produit-select js-single" id="produit" value="{{ old('produit') }}" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm">
                  ...
                  <option value="{{ $produit->Reference }}" data-designation="{{ $produit->Designation }}" data-type="{{ $produit->Type }}" data-produit_id="{{ $produit->id }}">{{ $produit->Designation}}({{ $produit->Reference }})</option>
                  @endforeach
                </select>
              </div>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
              Souhaitez-vous vraiment modifier ce proforma ?
            </div>
...
<script>
  ...
  $(document).ready(function() {
    ...
    // Remplacez ceci par l'identifiant de la facture réelle

    // Envoyer une requête AJAX pour récupérer les détails de la facture
    $.ajax({
      url: '/get-invoice-details/' +
        proforma_id, // L'URL de votre route pour récupérer les détails de la facture
      type: 'GET'
      , success: function(response) {
        // console.log('response', response)
        // Vérifiez si la réponse contient les détails des produits
        if (response.detailProforma && response.detailProforma.length > 0) {
          // Accédez aux détails du premier produit
          var firstProduct = response.detailProforma[0];


          // Ajoutez automatiquement une ligne au tableau avec les détails du premier produit

          // Si vous voulez ajouter des lignes pour tous les produits, bouclez à travers les détails du produit
          for (var i = 0; i < response.detailProforma.length; i++) {
            ...
            if (taxeCodeLettre == 'A') {
              ...
              totalExoneres += Math.round(montantHT * 1);
            ...
            if (taxeCodeLettre === 'B') {
              ...
              totalTVA_B += Math.round(ligneTVA_B);
              totalHT_B += Math.round(ligneHT_B);
              $('#totalTVA_B').val(totalTVA_B);
              $('#totalHT_B').val(totalHT_B);
            }
            if (taxeCodeLettre === 'C') {
                var montantHT = (mTtc/1) ;
              var ligneHT_C = Math.round(montantHT);
              totalHT_C += Math.round(ligneHT_C);
              $('#totalHT_C').val(totalHT_C);
            }
            if (taxeCodeLettre === 'D') {
                var montantHT = (mTtc/1.18) ;
              var ligneTVA_D =  parseFloat(mTtc.toFixed(0)) - parseFloat(montantHT.toFixed(0));
              var ligneHT_D = parseFloat(montantHT);
              totalTVA_D += Math.round(ligneTVA_D);
              totalHT_D +=Math.round(ligneHT_D);
            ...
            if (taxeCodeLettre === 'E') {
              ...
              totalHT_E += Math.round(ligneHT_E);
              $('#totalHT_E').val(totalHT_E.toFixed(0));
            }
            if (taxeCodeLettre === 'F') {
                var montantHT = (mTtc/1) ;
              var ligneHT_F = Math.round(montantHT);
              totalHT_F += Math.round(ligneHT_F);
              $('#totalHT_F').val(totalHT_F.toFixed(0));
            }
            var montantHT2 = Math.round(montantHT);
            updateOrAddRow(Reference, product.Designation, GroupeTaxation, quantite, pu_HT, tauxRemise, pu_HT_net, pu_TTC, Typeproduit,montantHT2);
            ...
            // console.log('totalHT_Global', totalHT_Global);
            ...
            if (selectedAibValue === '1' && carateresIFU.length == 13) {
            // console.log("AIB facturable est de 1%");
            aib_facturer = Math.round(totalHT_Global * 0.01);
            $('#aib_facturer').val(aib_facturer);

            } else if (selectedAibValue === '1' && carateresIFU.length < 13) {
            // console.log("AIB facturable est de 5%");
            aib_facturer =  Math.round(totalHT_Global * 0.05);
            $('#aib_facturer').val(aib_facturer);
          ...
          console.error('Aucun détail de produit trouvé dans la réponse AJAX.');
        }
      },
    ...
    if(montantHT2 != null){
     ...
     console.log('montantTTC', montantTTC);
      ...
      /* var selectedAibValue = $('input[name="Aib"]:checked').val();
        console.log('efefeeeeeeeeeeeeeeeeeeeeeeee Aib', selectedAibValue);
        var selectedClient = $('#client').find(':selected');
        var clientIFU = selectedClient.data('ifu');
        var clientIFU = selectedClient.data('ifu').toString();
        console.log('me voici clientIFU', clientIFU);
        var carateresIFU = clientIFU.split('');
        console.log('merde1', carateresIFU);
      ...
      if (rowToUpdate == null) {
        ...
        var clientIFU = selectedClient.data('ifu');
        var clientIFU = selectedClient.data('ifu').toString();
        // console.log('me voici clientIFUd', clientIFU);
        ...
        // console.log("Client sélectionné :", selectedClient.val());
        // console.log("IFU du client sélectionné :", clientIFU);
        // console.log("totalTTCValued :", totalTTC);
        ...
        //   console.log("AIB facturable est de 1%");
          aib_facturer = totalHT_Global * 0.01;
          aib_facturer = parseFloat(aib_facturer.toFixed(0));
          $('#aib_facturer').val(aib_facturer.toFixed(0));
