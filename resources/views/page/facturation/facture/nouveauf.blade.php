@extends('layouts.master', ['title' => 'Créer Facture'])
@section('content')
...
<section>
  ...
  <div class="d-flex flex-row-reverse bd-highlight">
    <div class="dropdown mb-2">
      <a href="{{ route('facture') }}" class="btn text-white" style="{{ background_color_1() }}">
  ...
  <form id="creeFactureForm" action="{{ route('storeFacture') }}" method="POST" class="">
    @csrf
        ...
        <fieldset class="border p-3 rounded-3">
          ...
          <div class="row">
            <div class="form-group col-md-6">
                <label for="client" class="form-label fw-bold position-relative">
                  Client
                ...
                <select class="form-select js-single" name="client_id" required id="client">
                  ...
                  @forelse ($listeClient as $client)
                    <option value="{{ $client->id }}" data-ifu="{{ $client->Numero_ifu }}" data-cat_client="{{ $client->Categorie_client_id }}" {{ isset($infoProforma) && $infoProforma->client_id == $client->id ? 'selected' : '' }}>
                      {{ $client->Denomination_sociale }} ({{ $client->Numero_ifu }})
                    </option>
                  @empty
                    <!-- Si aucun client n'est disponible, vous pouvez afficher un message ou une option par défaut -->
                    <option disabled>Aucun client disponible</option>
                  @endforelse
                </select>
                <div id="client-error-message" class="text-danger mt-2" style="display: none;">Veuillez sélectionner un client.</div>
              </div>
        ...
        <fieldset class="border p-3 rounded-3">
          ...
          <div class="row">
            <div class="col-md-6">
              ...
              <div class="btn-group w-100" role="group" aria-label="Basic radio toggle button group">
                <input type="radio" class="btn-check AibFacturer-select" name="Aib" {{ isset($infoProforma) && $infoProforma->Aib == 0 ? 'checked' : '' }} value="0" id="Aib0" @if (aibPrecocher() == 0) checked @endif >
                <label class="btn btn_aib" for="Aib0">NON</label>


                <input type="radio" class="btn-check AibFacturer-select" name="Aib" value="1" {{ isset($infoProforma) && $infoProforma->Aib == 1 ? 'checked' : '' }} id="Aib1" @if (aibPrecocher() == 1) checked @endif >
        ...
        <fieldset class="border p-3 rounded-3">
          ...
          <div class="row">
            <div class="form-group col-md-3">
              ...
              <select name="" type="text" class="form-select produit-select js-single" disabled id="produit" value="{{ old('produit') }}" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm">
            ...
            <div class="form-group col-md-3">
              ...
              <input type="number" min="1" class="form-control" value="{{ old('pu_HT') }}" id="pu_HT" aria-label="file example" >
                ...
                <input hidden type="number" min="1" class="form-control" value="{{ old('pu_HT_new') }}" id="pu_HT_new" aria-label="file example" >
            <div class="form-group col-md-3">
              <label for="validationTextarea" class="form-label fw-bold">%Remise</label>
              <input type="number" class="form-control " value="" id="Taux_remise" aria-label="file example">
            </div>
            <div class="form-group col-md-3">
      ...
      <div class="card-body">
        <fieldset class="border p-1 rounded-3">
          ...
          <div class="table-responsive">
            ...
            <div id="table-error-message" class="text-danger mt-2" style="display: none;">Veuillez ajouter au moins une ligne dans le tableau.</div>
          </div>
        </fieldset>
      </div>
    </div>
    ...
    <div class="mb-5">
        <button id="validate-button" class="btn text-white d-block ms-auto" style="background-color: #007bff;" type="button">Valider</button>
      </div>
    <br>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
                Voulez-vous sauvegarder cette facture ?
            </div>
            <div class="modal-footer">
              <button id="cancel-button" type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
              <button type="submit" id="save-button" class="btn " style="{{ background_color_2() }}">Continuer</button>
            </div>
          </div>
        </div>
      </div>
...
<script>
      $('#creeFactureForm').on('submit', function(e) {
...
<script>
    // Fonction pour vérifier et afficher/masquer le message d'erreur pour le champ client
    ...
    // Fonction pour vérifier et afficher/masquer le message d'erreur pour le tableau
    function validateTableInput() {
      var tableBody = document.querySelector('#table2 tbody');
      var tableErrorMessage = document.getElementById('table-error-message');
    ...
    document.getElementById('validate-button').addEventListener('click', function() {
      ...
      var isClientValid = clientSelect.value.trim() !== '';
      var isTableValid = tableBody.rows.length > 0;
      ...
      if (isClientValid && isTableValid) {
        var myModal = new bootstrap.Modal(document.getElementById('staticBackdrop'));
        myModal.show();
      }
    });

    // Écouteur d'événement pour le champ de sélection
    document.getElementById('client').addEventListener('input', validateClientInput);
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#add').click(function(selectedAib) {
      ...
      var pu_HT_new = parseFloat($('#pu_HT_new').val());
    ...
    function updateOrAddRow(reference, designation, taxe_id, quantite, pu_HT, Taux_remise, pu_HT_net
      , pu_TTC, magasin, StockeId, Typeproduit) {
    ...
    //   console.log('ffff', rows);
     // var montantHT = parseFloat(quantite) * parseFloat(pu_HT) * (1 - Taux_remise / 100);
      var montantTTC = parseFloat(quantite) * parseFloat(pu_TTC);
        ...
        if (taxeCodeLettre === 'A') {
            var mtHt = Math.round(montantTTC);
    ...
    //   console.log('efefeeeeeeeeeeeeeeeeeeeeeeee StockeId', StockeId);
      ...
      if (rowToUpdate == null) {
        ...
        if (taxeCodeLettre === 'B') {
            var puhtd= parseFloat(montantHT) / parseFloat(quantite);
           // var ligneTVA_B = parseFloat(montantHT) * 0.18;  //le vrai
            //var ligneTVA_B_sqte = parseFloat(puhtd) * 0.18;
        ...
        if (taxeCodeLettre === 'D') {
          // var ligneTVA_D = parseFloat(pu_HT) * parseFloat(quantite) * 0.18;
          var ligneTVA_D =parseFloat(montantTTC.toFixed(0)) - parseFloat(montantHT);
        //   console.log('ligneTVA_DligneTVA_DligneTVA_D', ligneTVA_D);
          var ligneHT_D = parseFloat(montantHT);
          totalTVA_D += ligneTVA_D;
          totalHT_D += ligneHT_D;
        //   console.log('totalHT_DtotalHT_D', totalHT_D);
        ...
        //Net_a_payer = totalTTC + aib_facturer - aibFloat;
        $('#Net_a_payer').val(Net_a_payer.toFixed(0));
      }
      ...
      if (rowToUpdate !== null) {
        ...
        console.log('pu_HT_new', pu_HT_new)
        ...
        var newRow = `<tr>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][produit]" class="form-control border-0" value="${reference}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][designation]" class="form-control border-0 designation-input" value="${designation}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][taxe_id]" class="form-control border-0" id="taxe_id" value="${taxe_id}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="number" style="width: 200px" name="inputs[${rowCount}][quantity]" class="form-control border-0" id="quantity" value="${quantite}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="number" style="width: 200px" name="inputs[${rowCount}][pu_HT]" class="form-control border-0" id="pu_HT" value="${pu_HT}" readonly>
                                </div>
                            </td>
                              <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="number" style="width: 200px" name="inputs[${rowCount}][pu_HT_new]" class="form-control border-0" id="pu_HT_new" value="${pu_HT_new}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="number" style="width: 200px" name="inputs[${rowCount}][Taux_remise]" class="form-control border-0" id="Taux_remise" value="${Taux_remise}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="number" style="width: 200px" name="inputs[${rowCount}][pu_HT_net]" class="form-control border-0" id="pu_HT_net" value="${pu_HT_net}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="number" style="width: 200px" name="inputs[${rowCount}][pu_TTC]" class="form-control border-0" id="pu_TTC" value="${pu_TTC}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][montantHT]" class="form-control border-0" id="montantHT" value="${montantHT}" readonly>
                                </div>
                            ...
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][montantTTC]" class="form-control border-0" id="montantTTC" value="${montantTTC}" readonly>
                                </div>
                            </td>
                            <td hidden>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][TypeP]" class="form-control border-0" id="TypeP" value="${TypeP}" readonly>
                                </div>
                            </td>
                            <td hidden>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][StockeId]" class="form-control border-0" id="StockeId" value="${StockeId}" readonly>
                                </div>
                            </td>
                            <td>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" style="width: 200px" name="inputs[${rowCount}][magasin]" class="form-control border-0" id="magasin" value="${magasin}" readonly>
                                </div>
                            </td>
