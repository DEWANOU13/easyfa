@extends('layouts.master', ['title' => 'Réceptionner Emballage'])
@section('content')
    @include('layouts.partials.entete-page', [
        'infos1' => 'Réceptionner  Emballage',
        'infos2' => 'Réceptionner  Emballage',
        'infos3' => 'Liste',
    ])
        ...
        <div class="col text-end">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" style="z-index: 2000;">
                    @can('receptionner-approvisionnement-emballage')
                        <li><a class="dropdown-item" type="button"
                                href="{{ route('nouveau_reception_approvissionner_emballage') }}">Nouveau</a>
                        </li>
                    @endcan
                    <form action="{{ route('reception_approv_imprimer_action_emballage') }}" method="POST" target="_blank">
                        @csrf
                        ...
                        <div class="col-md-2">
                            <div class="row border">
                                <form class="d-flex align-items-center mt-2" id="filterForm"
                                    action="{{ route('filterReceptionApproEmballage') }}" method="GET">
                                    <div class="row">
                        ...
                        <div class="col-md-10">
                            <div class="card m-b-30 my-3">
                                <div class="card-header" style="{{ background_color_2() }}">
                                    <h3 class="mt-2  d-inline-block text-dark">Liste réception des approvisionnements
                                        emballages</h3>
                                </div>
                                            ...
                                            <tbody>
                                                @forelse($receptions as $reception)
                                                    <tr style="cursor:pointer" class="clickable-row"
                                                        data-url="{{ route('get.receptionner_by_reception_emballage', ['id' => $reception->id]) }}">
                                                        <td hidden class="entree-produit"><input type="hidden"
                                                                value="{{ $reception->id }}"></td>
                                                        <td class="entree-produit">{{ $reception->NomAgence }}
                                                        </td>
                                                        <td class="entree-produit">
                                                            {{ \Carbon\Carbon::parse($reception->Date_Reception)->format('d/m/Y H:i') }}
                                                        </td>
                                                        <td class="entree-produit">
                                                            {{ $reception->Reference_Reception }}</td>
                                                        <td class="entree-produit">
                                                            {{ $reception->Observations }}</td>
                                                        <td class="entree-produit">{{ $reception->name }}</td>
                                                    </tr>
                                                @empty
                                                @endforelse
                                            </tbody>
                            ...
                            <div class="card m-b-30 my-3">
                                <div class="card-header" style="{{ background_color_2() }}">
                                    <h3 class="mt-2  d-inline-block text-dark">Détails réceptions approvisionnements
                                        emballages</h3>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive responsive-1">
                                        <div class="col-md-12">
                                            <table id=""
                                                ...
                                                <tbody>
                                                    @if (isset($receptionners))
                                                        @forelse($receptionners as $receptionner)
                                                            <tr style="cursor:pointer" class="clickable-row">
                                                                <td class="entree-produit">
                                                                    {{ $receptionner->Reference }}</td>
                                                                <td class="entree-produit">
                                                                    {{ $receptionner->Designation }}</td>
                                                                <td class="entree-produit">{{ $receptionner->Libelle }}
                                                                </td>
                                                                <td class="entree-produit">
                                                                    {{ $receptionner->NomMagasin }}
                                                                </td>
                                                                <td class="entree-produit">
                                                                    {{ $receptionner->Qte_Approvisionnee }}
                                                                </td>
                                                                <td class="entree-produit">
                                                                    {{ $receptionner->Qte_Receptionnee }}</td>
                                                            </tr>
                                                        @empty
                                                        @endforelse
                                                    @endif
                ...
                <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab"
                    ...
                    <div class="row my-4">
                        <div class="col-md-8 offset-md-2 mb-5">
                            <h3>LISTE DES RECEPTIONS APPROVISIONNEMENTS SUR UNE PERIODE</h3>
                            <form class="row gx-3 gy-2 pb-4" method="POST"
                                action="{{ route('reception_approv_impression_emballage') }}" target="_blank">
                                @csrf
                                <div class="col-sm-6">
                                ...
                                <div class="col-sm-6">
                                    <label class="form-label" for="agence">Magasin</label>
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <select name="categorie" type="text" class="form-select" id="categorie">
                                        ...
                                        @foreach (categorie_approvs() as $categorie)
        ...
        $(document).ready(function() {
            ...
            $('#categorie').on('change', function() {
                ...
                if (selectedCategoryId) {
                    $.ajax({
                        url: '/get-reception-approv/', // URL à adapter selon tes routes
                        type: 'POST',
                        data: {
