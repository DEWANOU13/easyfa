@extends('layouts.master', ['title' => 'Creer Réception Emballage'])
@section('content')
    @include('layouts.partials.entete-page', [
        'infos1' => 'Nouveau Réception Emballage',
        'infos2' => 'Nouveau Réception Emballage',
        'infos3' => 'Liste',
    ])
    <style>
        .entete_tableau {
    ...
    <div class="d-flex flex-row-reverse bd-highlight">
        <div class="dropdown mb-2">
            <a href="{{ route('reception_approvisionnement_emballage') }}" class="btn text-white"
    ...
    <div class="card m-b-30">
        <div class="card-header rounded" style="{{ background_color_2() }}">
            <h4 class="mt-2 text-dark">
                Créer une Réception Emballage
            </h4>
        </div>
    </div>
    <section>
        <form id="formcreate" action="{{ route('store.reception_approv_nouveau_emballage') }}" method="post">
            @csrf
            @if ($errors->any())
                ...
                $('.produit-select-produit').each(function() {
                    ...
                    if (receptionners.length > 0) {
                        var reglementsFacture = receptionners.filter(function(element) {
                            return element.Id_Ligne_Appro_Emballage === parseInt(idApprovisionner);
                        });
        ...
        $(document).ready(function() {
            ...
            $('#num_facture').on('change', function() {
                ...
                if (selectedCategoryId) {
                    $.ajax({
                        url: '/get-approvisionner-reception-emballage',
                        type: 'POST',
                        headers: {
                            ...
                            if (response.length > 0) {
                                response.forEach(function(produit) {
                                    $('#produit').append('<option value="' +
                                        produit.id + '|' +
                                        produit.Id_Stock_Emballage + '|' + produit
                                        .Designation +
                                        '">' +
