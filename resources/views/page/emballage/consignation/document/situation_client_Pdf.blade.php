...
<body>
    ...
    <h2 style="text-align: center; text-transform: uppercase; text-decoration: underline; margin-top: 100px ">RAPPORT VENTE</h2>
    ...
    <table class="tableListe">
        ...
        <tbody>
            @foreach ($data['tableSituationClientData'] as $row)
            @if (in_array($row['Libelle'], [
                'Ancienne dette','Dette récente'

            ]))
            <tr>
                <td colspan="5" style="font-weight: bold;">{{ $row['Libelle'] }}</td>
            </tr>
            @else
            <tr>

                <td >{{ $row['Libelle'] }}</td>
                <td style="">{{ $row['total_Qte'] }}</td>
                <td  style="">{{ $row['total_restituee'] }}</td>
                <td  style="">{{ $row['total_facturee'] }}</td>
                <td style="">{{ $row['restant'] }}</td>
            </tr>
