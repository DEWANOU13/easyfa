@extends('layouts.master', ['title' => 'Creer Consignation Entrée'])
@section('content')
    @include('layouts.partials.entete-page', [
        'infos1' => 'Nouveau Consignation Entrée',
        'infos2' => 'Nouveau Consignation Entrée',
        'infos3' => 'Liste',
    ])
    <style>
        .entete_tableau {
    ...
    <div class="d-flex flex-row-reverse bd-highlight">
        <div class="dropdown mb-2">
            <a href="{{ route('consignation_entree') }}" class="btn text-white" style="{{ background_color_1() }}">
    ...
    <div class="card m-b-30">
        <div class="card-header rounded" style="{{ background_color_2() }}">
            <h4 class="mt-2 text-dark">
                Créer une Consignation Entrée
            </h4>
        </div>
    </div>
    <section>
        <form id="formcreate" action="{{ route('storeDeconsignation_entree') }}" method="post">
            @csrf
            @if ($errors->any())
                    ...
                    <fieldset class="border p-3 rounded-3">
                        ...
                        <div class="row">
                            <div class="form-group col-md-12">
                                <label for="validationTextarea" class="form-label fw-bold position-relative">Fournisseur
                                    <span
                            ...
                            <div class="form-group col-md-12">
                                ...
                                <div class="input-group input-group-md mb-3">
                                    <select name="consignation_id" class="form-select js-single" id="consignation_imput"
                                        required>
                                        <option value="">Sélectionnez une entrée de consignation</option>

                                    </select>
                                    <div class="invalid-feedback">L'entrée est obligatoire</div>
                                </div>
                            </div>
                            <div class="form-group col-md-12">
                                <label for="validationTextarea" class="form-label fw-bold position-relative">Statut
                                    <span
                                        class="position-absolute top-0 start-100 translate-middle p-2 text-danger ms-1 mt-2">*
                                    </span>
                                </label>
                                ...
                                <div class="input-group input-group-md mb-3">
                                    <select name="statut" type="text" class="form-select js-single" id="statut"
                                        required>
                                        <option value="">Sélectionnez un fournisseur</option>
                                        <option value="PARTIELLEMENT DECONSIGNE">PARTIELLEMENT DECONSIGNE</option>
                                        <option value="DECONSIGNATION TERMINEE">DECONSIGNATION TERMINEE</option>
            ...
            <div class="card m-b-30">
                <div class="card-body">
                    <fieldset class="border p-3 rounded-3" id="table">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label fw-bold" for="num_facture">Emballage</label>
                                <div class="input-group input-group-sm mb-3">
                                    <select name="" type="text" class="form-select produit-select js-single"
                                        ...
                                        @foreach ($stock_emballages as $key => $value)
                                            <option value="{{ $value->id }}|{{ $value->Nom_Emballage }}|{{ $value->NomMagasin }}">
                                                {{ $value->Nom_Emballage }} => {{ $value->NomMagasin }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <label class="form-label fw-bold" for="net_a_payer">Qté en stock</label>
                                <div class="input-group input-group-sm mb-3">
                                    <input step="0.01" type="text" name=""
                                        class="form-control net-a-payer-input" value="{{ old('net_a_payer') }}"
                                        id="net_a_payer" placeholder="Qté en stock..." readonly>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label fw-bold" for="montant_regle">Qté déjà déconsignée</label>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="text" name="" value="{{ old('montant_regle') }}"
                                        id="montant_regle" class="form-control montant-regle-input"
                                        placeholder="Qté déjà déconsignée..." readonly>
                                </div>
                            </div>
                            {{-- <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <label class="form-label fw-bold" for="montant">Quantité déconsignée</label>
                                <div class="input-group input-group-sm mb-3">
                                    <input type="number" name="" class="form-control" placeholder="Quantité déconsignée..."
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var approvisionnements = @json($stock_emballages); // Convertir la variable PHP en JSON
        ...
        $(document).ready(function() {
            // Fonction pour activer/désactiver le bouton Valider en fonction des champs fournisseur et consignation_id
            function toggleSubmitButton() {
                var client = $('select[name="client_id"]').val();
                var consignation_id = $('select[name="consignation_id"]').val();

                // Si le fournisseur et l'consignation_id sont remplis, activer le bouton Valider, sinon le désactiver
                if (consignation_id && client) {
                    $('#bouton-valider').prop('disabled', false);
                } else {
                    $('#bouton-valider').prop('disabled', true);
                }
            }
            // Surveiller les événements de changement dans les champs fournisseur et consignation_id
            $('select[name="consignation_id"], select[name="client_id"]').on('input', function() {
                toggleSubmitButton();
            });
        ...
        $(document).ready(function() {
            ...
            function updateDesignation() {
                var approvisionnements = {!! json_encode($stock_emballages) !!};
                var receptionners = {!! json_encode($quantite_deja_receptionnee) !!};

                console.log('receptionners', receptionners)
                // console.log('approvisionnements', approvisionnements)

                var consignation_id = $('#consignation_imput').val();

                console.log('consignation_id', consignation_id);

                $('.produit-select').each(function() {
                    var id_stock_emballage = $(this).val();
                    var parties = id_stock_emballage.split("|");
                    var selectedReference = parties[1];
                    var idApprovisionner = parties[0];

                    console.log(parties)
                    console.log(id_stock_emballage)
                    ...
                    var getFacture = approvisionnements.find(function(element) {
                        return element.id === parseInt(id_stock_emballage);
                    });
                    ...
                    if (receptionners.length > 0) {
                        ...
                        if (getFacture) {
                            ...
                            resteAPayer.val(getFacture.Qte_stockee);
                        } else {
                            netAPayer.val('');
                            montantRegle.val('');
                            resteAPayer.val('');
            ...
            $('#add').click(function() {
                ...
                if (!isNaN(montant)) {
                    if (sommeQuantites > resteAPayer) {
                        ...
                        updateOrAddRow(numFacture, produit, netAPayer, montantRegle, resteAPayer,
                            modeReglement,
                            montant);
                        clearFormFields();
                        toggleSubmitButton();
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#client_input').change(function() {
                ...
                if (clientId) {
                    $.ajax({
                        url: '/getConsignationDuClient-entree/' + clientId,
                        type: 'GET',
