@props(['title' => __('Confirmez le mot de passe'), 'content' => __('Pour votre sécurité, veuillez confirmer votre mot de passe pour continuer.'), 'button' => __('Confirmer')])
...
@once
<x-dialog-modal wire:model.live="confirmingPassword" class="flex items-center">
  <x-slot name="title">
    {{ $title }}
  </x-slot>
  ...
  <x-slot name="content">
    ...
    <div class="mt-4" x-data="{}" x-on:confirming-password.window="setTimeout(() => $refs.confirmable_password.focus(), 250)">
      <x-input type="password" class="mt-1 block w-3/4" placeholder="{{ __('Mot de passe') }}" autocomplete="current-password" x-ref="confirmable_password" wire:model="confirmablePassword" wire:keydown.enter="confirmPassword" />
